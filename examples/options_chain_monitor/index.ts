...
function renderTable(
	...
	const table = new Table({
		...
		colAligns: ["right", "center", "right", "right", "right", "right", "right", "right", "right"],
	...
	const header =
		...
		chalk.dim(`  —  ${current.length} contracts  —  ${updatedAt.toLocaleTimeString()}`);
	return `${header}\n${table.toString()}`;
}
...
async function main(): Promise<void> {
	...
	if (!underlying || !expiration) {
		console.error("Usage: pnpm monitor <UNDERLYING> <EXPIRATION> [INTERVAL_SECONDS]");
	...
	while (!stopped) {
		try {
			...
			clearScreen();
			console.log(renderTable(underlying, expiration, rows, previous, new Date()));
