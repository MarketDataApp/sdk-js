...
Four candlestick examples on a common dataset (1 year of AAPL daily candles). Each script writes a self-contained HTML file next to itself and opens it in your default browser. The chart library is loaded from a public CDN — there are no per-library `npm install` steps.
...
> Chart.js isn't in this set because it has no native candlestick type — it needs the `chartjs-chart-financial` plugin. It's used below for an options-chain example instead.

## Other examples

| Script | Command | What it does |
|---|---|---|
| [`charts/chartjs`](./charts/chartjs) | `pnpm chart:chartjs` (or `pnpm chart:chartjs TSLA`) | Fetches the next-monthly options chain and renders an interactive Chart.js page — calls vs puts plotted across strikes, with an in-page dropdown to switch between IV, Delta, Gamma, Theta, and Vega without a refetch. |
