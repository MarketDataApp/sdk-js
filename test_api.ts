...
async function main() {
	...
	if (optionsChain.isErr()) {
		...
		if (data.length > 0) {
			...
			if (unexpiredOption) {
				...
				optionSymbol = unexpiredOption.Symbol as string;
			} else {
				console.log(
					"⚠️ No unexpired option found in chain, using first available (Test 14 might fail)",
				);
	...
	if (optionsStrikes.isErr()) {
		...
		console.log(
			"✓ Success! Got strikes for",
			keys.length,
			"expiration date(s)",
		);
		if (keys.length > 0)
			console.log(
				`First date (${keys[0]}) strikes:`,
	...
	if (sdk.rateLimits) {
		...
		console.log(
			`  Remaining: ${sdk.rateLimits.requestsRemaining}/${sdk.rateLimits.requestsLimit}`,
		);
		console.log(
			`  Reset: ${new Date(sdk.rateLimits.requestsReset * 1000).toLocaleTimeString()}`,
		);
	}
}
